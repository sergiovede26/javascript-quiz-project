...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showQuestion() {
    ...
    let questionPercent = ((quiz.currentQuestionIndex + 1) / quiz.questions.length) * 100;

    progressBar.style.width =  `${questionPercent}%`; // This value is hardcoded as a placeholder
    ...
    question.choices.forEach((choice) => {
      ...
      radLabel.innerHTML = choice;

      newRadio.setAttribute("type", "radio");
      newRadio.setAttribute("name", "choice");
      newRadio.setAttribute("value", choice);

      choiceContainer.appendChild(newRadio);
      choiceContainer.appendChild(radLabel);
